...
const TIER_COLORS = {
  critical:     { bg: "from-red-600 to-rose-500",    ring: "#ef4444" },
  "needs-work": { bg: "from-orange-500 to-amber-400", ring: "#f97316" },
  good:         { bg: "from-yellow-500 to-lime-400",  ring: "#eab308" },
  excellent:    { bg: "from-emerald-500 to-green-400",ring: "#22c55e" },
  outstanding:  { bg: "from-blue-500 to-indigo-400",  ring: "#3b82f6" },
}

const SHARE_URL = "https://pocket-app.com"
...
export function AchievementCard({ open, onClose }: Props) {
  ...
  // No score yet → not enough data. Don't fabricate a "Good / 0" card that
  // would misrepresent an empty account on a shareable image. (RA-18)
  if (!healthScore) {
    return (
      <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-base">{t("shareYourStats")}</DialogTitle>
          </DialogHeader>
          <p className="py-6 text-center text-sm text-muted-foreground">{t("notEnoughData")}</p>
        </DialogContent>
      </Dialog>
    )
  }
  ...
  const savedDisplay = formatSavingsRate(currentMonthIncome, currentMonthExpenses)
  ...
  const shareImageNative = async () => {
    ...
    try {
      ...
      if (navigator.canShare?.({ files: [result.file] })) {
        await navigator.share({ files: [result.file], title: t("shareTitle") })
      } else {
        ...
        a.click()
        URL.revokeObjectURL(url)
        toast.info(t("imageSaved"))
      }
    } catch (e: any) {
      if (e?.name !== "AbortError") toast.error(t("shareFailed"))
    } finally {
      setCapturing(false)
    }
  }
  ...
  const shareToTwitter = () => {
    const text = t("shareScoreText", { score, tier: tierLabel })
    window.open(
  ...
  const shareToWhatsApp = () => {
    const text = t("shareStatsText", { month, score, tier: tierLabel })
      ...
      <DialogContent className="max-w-sm">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6 relative">
            ...
            <div className="text-right">
              <p className="text-xs opacity-75">{t("healthScoreLabel")}</p>
              <p className="text-3xl font-black leading-none">{score}</p>
              <p className="text-xs font-semibold opacity-90">{tierLabel}</p>
            </div>
          </div>

          {/* Stats row */}
          <div className="grid grid-cols-3 gap-3 relative">
            <StatBox label={t("income")} value={formatAmount(currentMonthIncome)} />
            <StatBox label={t("expenses")} value={formatAmount(currentMonthExpenses)} />
            <StatBox
              label={t("saved")}
              value={savedDisplay}
          ...
          <div className="mt-4 pt-3 border-t border-white/20 relative flex items-center justify-between">
            ...
            <p className="text-xs opacity-60">{t("tagline")}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-4 gap-2">
            <SocialButton label="Facebook" ariaLabel={t("shareTo", { platform: "Facebook" })} disabled={capturing} onClick={shareToFacebook}>
            ...
            <SocialButton label="Instagram" ariaLabel={t("shareTo", { platform: "Instagram" })} disabled={capturing} onClick={shareToInstagram}>
            ...
            <SocialButton label="X / Twitter" ariaLabel={t("shareTo", { platform: "X / Twitter" })} disabled={capturing} onClick={shareToTwitter}>
            ...
            <SocialButton label="WhatsApp" ariaLabel={t("shareTo", { platform: "WhatsApp" })} disabled={capturing} onClick={shareToWhatsApp}>
